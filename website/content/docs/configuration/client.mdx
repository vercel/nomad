...
- `cpu_core_compute` `(int: 0)` - Specifies the CPU compute that the scheduler
  charges for each core that a task reserves with `cores`. This value must use
  the same unit as `cpu_total_compute`, which must also be set. If this value is
  zero, Nomad charges the detected MHz of each selected core.
